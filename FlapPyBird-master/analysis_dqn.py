...
async def train_dqn_agent(display=False):
    ...
    while True:
        ...
        if agent.total_episodes % 10 == 0:
            ...
            avg_score = sum(agent.episode_scores[-10:]) / 10

            max_score = max(agent.episode_scores) if agent.episode_scores else 0 
            agent.max_scores.append(max_score)

            # Compute rolling mean score (moving average)
            window_size = 100
            rolling_mean_score = pd.Series(agent.episode_scores).rolling(window=window_size).mean().tolist()
            agent.rolling_mean_scores = rolling_mean_score

            # Save scores and metrics for analysis
            agent.training_data = {
            ...
            with open("training_metrics.json", "w") as f:
                json.dump(agent.training_data, f)

            print(f"Episode {agent.total_episodes}, Avg Reward: {avg_reward:.2f}, Max Score: {max_score}, Exploration: {agent.epsilon:.4f}")
